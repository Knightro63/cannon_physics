...
class ConvexPolyhedron extends Shape {
  ...
  void computeNormals() {
    final int len = faces.length;
    if(len > 0){
      faceNormals = List.filled(len, Vec3());//.length = faces.length;
    }
  ...
  void clipAgainstHull(
    ...
    int len = hullB.faces[closestFaceB]?.length ?? 0;
    for (int i = 0; i < len; i++) {
  ...
  void clipFaceAgainstHull(
    ...
    for (int i = 0; i < hullA.faces.length; i++) {
      int len = hullA.faces[i]?.length ??0;
      for (int j = 0; j < len; j++) {
        if (
          /* Sharing a vertex*/
  ...
  void updateBoundingSphereRadius() {
    ...
    for (int i = 0; i != verts.length; i++) {
      final norm2 = verts[i]?.lengthSquared() ?? 0;
      if (norm2 > max2) {
        max2 = norm2;
      }
    }
  ...
  void transformAllPoints([Vec3? offset, Quaternion? quat]) {
    ...
    if (quat != null) {
      // Rotate vertices
      for (int i = 0; i < n; i++) {
        final v = verts[i] ?? Vec3();
        quat.vmult(v, v);
      }
      // Rotate face normals
      for (int i = 0; i < faceNormals.length; i++) {
        final v = faceNormals[i] ?? Vec3();
        quat.vmult(v, v);
      }
      /*
            // Rotate edges
  ...
  static void project(ConvexPolyhedron shape, Vec3 axis, Vec3 pos, Quaternion quat, List<double> result) {
    ...
    min = max = vs[0]?.dot(localAxis) ?? 0;

    for (int i = 1; i < n; i++) {
      final val = vs[i]?.dot(localAxis) ?? 0;
      if (val > max) {
        max = val;
      }
      if (val < min) {
