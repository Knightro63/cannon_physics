...
class _TestBasicPageState extends State<TestBasic> {
  ...
  void initcannonPhysics(){
    world = cannon.World(cannon.WorldConfigure(isStat:true, scale:100.0));
    populate(type);
  }
  ...
  void populate(n) {
    ...
    //add ground
    world!.add(
      cannon.ObjectConfigure(
      shapes: [cannon.Shapes.box],
    ...
    for(int i = 0; i < max;i++){
      ...
      if(t==1){
        ...
        mat.color = randColor;
        bodys.add(world!.add(cannon.ObjectConfigure(
          shapes:[cannon.Shapes.sphere], 
